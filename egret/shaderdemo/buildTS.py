...
#  exml   不需要通过编译  libs 目录下 也不需要编译  只需要考虑 ts  就行了
def shortMD5(md5):
...
if __name__ == '__main__':
    print("begin build ts to js")
    filee = os.path.realpath(__file__)
    fpath,fname = os.path.split(filee)
    egretProFile = fpath + "/SmartpikachuGame/"
    allTSFile = GetFileList(egretProFile+ "src/",[])
    # 去掉开头
    rep = []
    for k in range(len(allTSFile) ):
        rep.append(allTSFile[k].replace(fpath, ""))
    allTSFile = rep
    manifest_tsJSONFile = fpath + "/manifest_ts.json"
